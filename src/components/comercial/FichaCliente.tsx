...
import { SeletorVisao, useVisaoRelatorio } from '@/components/comercial/SeletorVisao';
import type {
...
function VisaoSimplificada({
  ficha, de, ate, criterio, cashback, cashbackFalhou, onVerTudo,
}: {
  ficha: FichaCliente; de: string; ate: string; criterio: CriterioCurva;
  cashback: CashbackResumo | null; cashbackFalhou: boolean; onVerTudo: () => void;
}) {
  return (
    <div className="space-y-4">
      <Farois indicadores={ficha.indicadores} cashback={cashback} cashbackFalhou={cashbackFalhou} />
      ...
      <div className="grid gap-4 lg:grid-cols-3">
        ...
        <ListaCurta
          ...
          // Ordenado pelo que gira para os OUTROS clientes — é o que faz a
          // lista valer uma ligação, e por isso o valor vem junto.
          linhas={ficha.nunca_comprou.map((l) => ({ chave: l.produto_codigo, nome: l.nome, direita: formatBRL(l.valor_outros) }))}
          onVerTudo={onVerTudo}
        />
      </div>
    </div>
...
function Farois({
  indicadores, cashback, cashbackFalhou,
}: { indicadores: FichaClienteIndicadores; cashback: CashbackResumo | null; cashbackFalhou: boolean }) {
  ...
  return (
    <div className="grid gap-3 sm:grid-cols-2 xl:grid-cols-4">
      ...
      <Farol
        titulo="Cashback"
        // `cashback` nulo não é zero. Sem linha (ou com leitura falhada) o
        // farol diz "—"; `formatBRL` escreveria "R$ 0,00" e afirmaria que o
        // cliente não ganhou nada, que é outra coisa.
        valor={cashbackFalhou || !cashback || cashback.cashback === null ? '—' : formatBRL(cashback.cashback)}
      >
        {cashbackFalhou ? (
      ...
      {mix.length === 0 || total === 0 ? (
        ...
        <div className="space-y-2.5">
          {mix.map((m) => {
            ...
            const pct = m.participacao ?? (total === 0 ? 0 : (m.valor / total) * 100);
            return (
