...
// `stripe-signature`, conferida com STRIPE_WEBHOOK_SECRET. O Stripe pode
...
Deno.serve(async (req) => {
  ...
  if (!stripeKey || !webhookSecret) return json({ error: 'stripe_not_configured' }, 503);
  ...
  const { error: dedupeError } = await admin.from('crm_stripe_events').insert({ event_id: event.id, type: event.type });
  if (dedupeError) {
    if (dedupeError.code === '23505') return json({ received: true, duplicate: true });
    console.error('crm_stripe_events insert', dedupeError);
  ...
  await admin.from('crm_stripe_events').update({ tenant_id: order.tenant_id, order_id: order.id }).eq('event_id', event.id);
