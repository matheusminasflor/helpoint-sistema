...
<body style="margin:0;padding:0;background:#f5f6f8;font-family:'Segoe UI',Tahoma,Verdana,sans-serif;">
  <div style="max-width:560px;margin:0 auto;padding:40px 20px;">
    <div style="background:#fff;border-radius:14px;padding:36px;box-shadow:0 2px 10px rgba(0,0,0,.06);">
      <div style="text-align:center;color:#1a1a2e;font-size:20px;font-weight:700;margin-bottom:6px;">HelpPoint</div>
      ...
      <p style="text-align:center;color:#626d80;font-size:15px;line-height:1.6;margin:0 0 24px;">
        Confirme seu e-mail para concluir o cadastro da sua empresa no HelpPoint.
      </p>
    ...
    <div style="text-align:center;margin-top:18px;color:#94a0b3;font-size:11px;">© ${new Date().getFullYear()} HelpPoint</div>
  </div>
</body></html>`;
}
...
Deno.serve(async (req) => {
  ...
  try {
    ...
    const { error: sendErr } = await resend.emails.send({
      from: `HelpPoint <${fromAddress}>`,
      to: [e],
      subject: 'Confirme seu e-mail no HelpPoint',
