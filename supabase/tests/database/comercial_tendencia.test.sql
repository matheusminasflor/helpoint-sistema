...
-- 1. Novo — sem venda na 1ª metade, com venda na 2ª. Mutação: trocar a
-- ordem do CASE pondo a checagem de Esporádico antes de Novo (o produto
-- tem só 1 de 12 meses com venda, cairia em Esporádico).
select is(
...
-- 2. Descontinuado — vendia na 1ª metade, zerado na 2ª. Mesma mutação de
-- ordem do teste acima quebraria este também (1 de 12 meses).
select is(
...
-- da 1ª — ainda assim Esporádico, não Crescendo. Mutação: mover a checagem
-- de Esporádico para DEPOIS da de Crescendo/Caindo/Estável no CASE.
select is(
...
-- 4. Crescendo — 2ª metade ≥25% acima da 1ª (aqui, +50%). Mutação: trocar
-- `>=` por `>` no limiar de 1.25 (não muda o resultado deste caso — a
-- mutação que mata é trocar `1.25` por um valor > 1.5, ex. `1.6`).
select is(
...
-- num único mês. Mutação: trocar `> m.total_valor * 0.5` por `>=`, ou trocar
-- o `0.5` por `0.6` (deixaria de marcar 60% exatos como concentrado se o
-- limiar subisse).
select is(
...
-- situação e variação saem nulas, nunca 'Estável'. Mutação: tirar a
-- checagem `v_total_meses <= 1` do CASE de situação.
select is(
...
-- e não 1200. Mutação: trocar `valor_curva` por `valor_nota` na CTE do
-- mensal (devolução deixaria de abater).
select is(
...
-- mas aparece quando p_filial é nulo (as duas). Mutação: ignorar p_filial
-- na CTE de clientes.
select is(
