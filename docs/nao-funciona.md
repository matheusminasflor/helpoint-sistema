...
- **CRM-2b (2026-09-12), ressalvas conhecidas:** (a) o caminho Bling **não foi
  ...
  `bling_order`, ao vivo no teste); (b) as respostas de `gerar-nfe` e de `GET
  /nfe/{id}` são assumidas com envelope `data` (`id`, `chaveAcesso`, `linkDanfe`)
  — se vierem diferentes, o passo grava `nfe_status = 'error'` com a mensagem e
  o fluxo tenta de novo (`retry: 2` no modelo); (c) a forma de pagamento do
...
- Cobertura de teste: 50 testes no front (Vitest) — SLA em `src/types/helpdesk.test.ts`, rotas
  ...
  14 sobre a base do CRM, 16 sobre funis editáveis, 25 sobre segmentos, tabelas de preço e portões, 17 sobre pedido e proposta, 8 sobre chaves de pagamento por empresa (CRM-2a), 7 sobre a conexão com o Bling e o passo `bling_order` (CRM-2b), 13 sobre campos
  personalizados, 13 sobre importação de planilha e 9 sobre indicadores de
  venda — **242**. O CI os roda contra um banco do zero a cada push ao
